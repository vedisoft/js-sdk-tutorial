...
```bash
[events off]> License 1 10 tinyCRM
```

> Первый аргумент команды License говорит о том, что лицензии нужно добавить (1) или удалить (0). Второй аргумент — количество пользователей, которые могут подключаться к серверу одновременно. Третий аргумент — тип клиентского приложения (напрмер, excel или 1C).
